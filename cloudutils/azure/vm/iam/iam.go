...
// GetAuthorizerFromCertificate get authorizer based on certificate
func GetAuthorizerFromCertificate(path string, password string, clientID string, tenantID string) autorest.Authorizer {
	fmt.Println(path, password, clientID, tenantID)
	certificateConfig := auth.NewClientCertificateConfig(path, password, clientID, tenantID)
	auth, err := certificateConfig.Authorizer()
	if err != nil {
		log.Fatal(err)
	}
	return auth
...
// GetAuthFromEnv get authorization info from environment
func GetAuthFromEnv() autorest.Authorizer {
